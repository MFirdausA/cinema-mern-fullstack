...
export default function CustomerWalletTopup() {
    ...
    const handleTopup = async () => {
        try {
            if (balance === 0) {
                alert("Please choose an amount to top up.");
                return;
            }
        ...
        <div id="Content-Container" className="relative flex flex-col w-full max-w-[640px] min-h-screen mx-auto text-white bg-[linear-gradient(179.86deg,_#000000_40.82%,_#0E0E24_99.88%)] overflow-x-hidden">
            <div id="Header" className="flex flex-col gap-5">
                <div id="Top-Nav" className="relative flex items-center justify-between px-5 mt-[60px]">
                    <a href="my-wallet.html" className="w-12 h-12 flex shrink-0 items-center justify-center bg-[#FFFFFF1A] backdrop-blur-md rounded-full">
            ...
            <p id="Amount-Choosed" className="font-bold text-4xl leading-[54px] text-center mt-[30px]">{balance === 0 ? "____" : rupiahFormat(balance)}</p>
            <form action="midtrans.html" className="relative px-5 mt-[30px]">
                <div id="Theaters" className="tab-content flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        key={`${item + i}`} 
                        className="group relative theather-card flex flex-col rounded-3xl p-4 gap-[2px] bg-white/10 backdrop-blur-md hover:bg-[#5033F1] has-[:checked]:bg-[#5033F1] transition-all duration-300">
                            <input type="radio" name="amount" className="absolute top-1/2 left-1/2 opacity-0" value={item} required />
                ...
                <div className="relative h-[98px] w-full max-w-[640px]">
                    ...
                    disabled={isPending}
                    onClick={handleTopup}
                    className="fixed bottom-[30px] w-[calc(100%-40px)] max-w-[600px] rounded-full p-[12px_18px] h-fit text-black bg-white font-bold text-premiere-black">Proceed to Topup</button>
                </div>
            </form>
        </div>
    )
